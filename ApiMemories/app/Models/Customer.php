...
class Customer extends Model
{
    ...
    {
        return HotelBookingResource::collection(HotelBooking::where('status', 1)->where('customer_id', $this->id)
    ...
    {
        return RestoBookingResource::collection(RestoBooking::where('status', 1)->where('customer_id', $this->id)
                                                                                ->orderBy('created_at', 'DESC')
                                                                                ->get());
    }
    ...
    public function restoReview()
    {
        return
            ...
            HotelReviewResource::collection($this->hasMany(RestoReview::class)
                                                 ->orderBy('created_at', 'DESC')
                                                 ->get());
    }
    ...
    {
        return TourBookingResource::collection(TourBooking::where('status', 1)->where('customer_id', $this->id)
                                                                              ->orderBy('created_at', 'DESC')
                                                                              ->get());
    }
